...
type IListData = Array<string>

const Lists = () => {
    const [listData, setListData] = useState<IListData>([])

    useEffect(() => {
        fetch('http://localhost:2000/session/all', {
            cache: 'no-store',
            method: 'get',
            headers: {
    ...
    return (
        <>
            ...
            <div className="gap-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
                {listData.length > 0 ? listData.map((value, index) => {
                    return (
                        <div key={index} className="rounded-lg bg-gray-400/10 p-3 shadow-md hover:bg-gray-400/20 sm:p-3 md:p-4">
                            <div className="mb-3">
                    ...
                    <div className="rounded-lg bg-rose-400/10 p-3 shadow-md hover:bg-rose-400/20 sm:p-3 md:p-4">
                        <div className="text-base font-semibold text-rose-500 uppercase">
                            Data Kosong
                        </div>
                    </div>
